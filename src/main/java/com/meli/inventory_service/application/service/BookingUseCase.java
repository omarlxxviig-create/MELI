...
public class BookingUseCase implements BookingPort {
    ...
    public Reservation createReservation(@Valid CreateReservationCommand command) {
        return reservationDurationTimer.record(() -> {
            ...
            // Lock post para actualización atómica
            ServicePost post = servicePostPort.findByIdForUpdate(command.getPostId())
                    .orElseThrow(() -> new IllegalArgumentException("Post not found: " + command.getPostId()));

            // Validar estado y disponibilidad
            if (!post.hasAvailableSeats(command.getSeats())) {
                logger.warn("Insufficient seats for reservation. Post: {}, Requested: {}, Available: {}",
                        command.getPostId(), command.getSeats(), post.getSeatsAvailable());
                reservationConflictsCounter.increment();
                throw new IllegalStateException(
                        String.format("Insufficient seats. Requested: %d, Available: %d",
                                command.getSeats(), post.getSeatsAvailable()));
            }

            // Decrementar asientos disponibles
            post.reserveSeats(command.getSeats());
    ...
    public Reservation cancelReservation(String reservationId) {
        ...
        ServicePost post = servicePostPort.findByIdForUpdate(reservation.getPostId())
                .orElseThrow(() -> new IllegalArgumentException("Post not found: " + reservation.getPostId()));
    ...
    public Reservation confirmReservation(String reservationId) {
        ...
        reservation.confirm();
        Reservation confirmed = reservationPort.save(reservation);

        createOutboxEvent("Reservation", reservationId, "RESERVATION_CONFIRMED",
                String.format("{\"reservationId\":\"%s\",\"userId\":%d}",
            ...
            if (reservation.getStatus() == ReservationStatus.PENDING) {
                try {
                    ...
                    createOutboxEvent("Reservation", reservation.getId(), "RESERVATION_EXPIRED",
                            String.format("{\"reservationId\":\"%s\"}", reservation.getId()));
